                                      ...
                                      int64_t* createTime)
{
  ...
  if (fd_ >= 0)
  {
    ...
    {
      struct stat statbuf;
      if (::fstat(fd_, &statbuf) == 0)      //fstat获取文件状态，将状态存入statbuf
      {
        if (S_ISREG(statbuf.st_mode))       //判断是否为一般文件
        {
          *fileSize = statbuf.st_size;
          content->reserve(static_cast<int>(std::min(implicit_cast<int64_t>(maxSize), *fileSize)));     //预留空间
        }
    ...
    while (content->size() < implicit_cast<size_t>(maxSize))
    {
      ...
      ssize_t n = ::read(fd_, buf_, toRead);        //读取数据到buf_中
      if (n > 0)
      {

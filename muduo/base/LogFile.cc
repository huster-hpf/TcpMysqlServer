...
// not thread safe
class LogFile::File : boost::noncopyable        //FIle¿¿¿
{
 public:
  ...
  void append(const char* logline, const size_t len)      //¿logline¿¿¿¿¿¿
  {
    ...
    while (remain > 0)
    {
      size_t x = write(logline + n, remain);    //remain¿¿¿¿x
      if (x == 0)
      {
        int err = ferror(fp_);
        if (err)
